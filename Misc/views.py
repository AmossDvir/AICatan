...
class BoardFrame(tkinter.Frame):
    def __init__(self, master, game, *args, **kwargs):
        ...
        # print(self._board.tiles[0].)

        board_canvas = tkinter.Canvas(self, width=CANVAS_WIDTH, height=CANVAS_HEIGHT, background='#749CEC')
...
class StartGamePlayerOrderFrame(tkinter.Frame):
    ...
    def __init__(self, master, game, *args, **kwargs):
        ...
        defaults = ('yurick green', 'josh #3D588E', 'zach orange', 'ross red')
...
class GameToolbarFrame(tkinter.Frame):
    ...
    def __init__(self, master, game, *args, **kwargs):
        ...
        self.label_cur_player_name = tkinter.Label(self, textvariable=self._cur_player_name, anchor="center",bg=self.game.get_cur_player().color,fg="white",font=(None, 13))
        ...
        frame_cards = CardsFrame(self,self.game)

        self.label_cur_player_name.pack(fill=tkinter.X)
        ...
        frame_end_game.pack(fill=tkinter.BOTH)
        frame_cards.pack(side=tkinter.BOTTOM,fill=tkinter.BOTH)
    ...
    def notify(self, observable):
        if self._cur_player.color != self.game.get_cur_player().color:
            self.set_cur_player_name()
            self.label_cur_player_name.configure(bg=self.game.get_cur_player().color)

    def set_cur_player_name(self):
        self._cur_player = self.game.get_cur_player()
        self._cur_player_name.set('Current Player: {0} '.format(
...
class RollFrame(tkinter.Frame):
    ...
    def __init__(self, master, game, *args, **kwargs):
        ...
        self.random_roll = tkinter.Button(self.smallnumbers, command=lambda:self.roll_event_HO(2), text='Roll Dice')
        self.random_roll.grid(row=0,column=0)
    ...
    def roll_event_HO(self, roll):
        ...
        self.on_roll(dice_one+dice_two)
    ...
    def set_states(self):
        ...
        self.random_roll.configure(state=can_do[self.game.state.can_roll()])
...
class CardsFrame(tkinter.Frame):
    def __init__(self,master,game):
        super(CardsFrame, self).__init__(master)
        self.master = master
        self.game = game

        self.cards_lbl = tkinter.Label(self,text="Current Cards: ")
        self.cards_lbl.grid()
