...
class Hexagon:
    def __init__(self, row, col, canvas):
        self.row = row
        self.col = col
        self.points = [None, None, None, None, None, None]  # NE, E, SE, SW,
        # W, NW
        ...
        if self.type != 'desert':
            ...
            self.dice_throw = -1 # desert tile is -1
        ...
        self.dice_throw_text = canvas.create_text(x - 0.5 * a, y - 0.5 * h,
                                                  text=self.dice_throw,
                                                  fill='black')
        self.canvas_hexagon = canvas.create_polygon(x - a, y, x, y, x + 0.5 * a,
                                                    y - 0.5 * h, x, y - h,
                                                    x - a, y - h, x - 1.5 * a,
                                                    y - 0.5 * h, fill=
                                                    Hexagon.dic_of_type_to_color[
                                                        self.type],
                                                    outline='black')
        canvas.tag_lower(self.canvas_hexagon)
        canvas.tag_bind(self.canvas_hexagon, '<Button-1>',
                        self.click_on_hexagon)
        ##############################################

    thieves = None
    dic_of_type_to_color = {'tree': '#016A22', 'wheat': '#FCFF45',
                            'sheep': '#A4FE74', 'clay': '#A73000',
                            'stone': '#4873C1', 'desert': 'black'}
    list_of_dice_throw = [6, 5, 9, 4, 3, 8, 10, 6, 5, 9, 12, 3, 2, 10, 11, 11,
                          4, 8]
    list_of_areas = ['tree', 'tree', 'tree', 'tree', 'wheat', 'wheat', 'wheat',
                     'wheat', 'sheep', 'sheep', 'sheep', 'sheep', 'clay',
                     'clay', 'clay', 'stone', 'stone', 'stone', 'desert']
    board = []
    ...
    def click_on_hexagon(self, event):
        if Player.move_thieves == True:
            ...
            if Hexagon.thieves.type == 'desert':
                ...
                Hexagon.canvas.itemconfig(Hexagon.thieves.dice_throw_text,
                                          fill='black')
            Hexagon.thieves = self
            Hexagon.canvas.itemconfig(Hexagon.thieves.dice_throw_text,
                                      fill='red')
    ...
    def create_board_of_hexagon(self, hexagon_canvas):
        ...
        for col in range(0, 5):
            ...
            for row in range(0, col_to_row[col]):
                temp_row.append(Hexagon(2 * row + col_to_start_index[col], col,
                                        hexagon_canvas))
                index += 2
            ...
            for hexagon in row:
                for index in range(0, 6):
                    if hexagon.points[index] == None:
                        hexagon.points[index] = Point(canvas, hexagon.row,
                                                      hexagon.col, index)
        ...
        for row in Hexagon.board:
            for hexagon in row:
                for point_index in range(0, 6):
                    if point_index == 5:
                        Point.set_road_between_two_points(Point, canvas,
                                                          hexagon.points[5],
                                                          hexagon.points[0],
                                                          point_index)
                    else:
...
class Point:
    ...
    def __init__(self, canvas, row, col, index):
        ...
        if index == 0:
            ...
            self.point_on_canvas = canvas.create_oval(self.x1, self.y1, self.x2,
                                                      self.y2, fill='white',
            ...
            self.point_on_canvas = canvas.create_oval(self.x1, self.y1, self.x2,
                                                      self.y2, fill='white',
                                                      outline='black', width=1)
        canvas.tag_bind(self.point_on_canvas, '<Button-1>',
                        self.click_point_function)
        #############################################

    def set_radius(self, radius):
        self.x1 = self.x - radius
        self.y1 = self.y - Point.h - radius
        self.x2 = self.x + radius
        self.y2 = self.y - Point.h + radius
        self.point_on_canvas = Point.canvas.create_oval(self.x1, self.y1,
                                                        self.x2, self.y2,
                                                        fill='white',
                                                        outline='black',
                                                        width=1)
    ...
    def set_road_between_two_points(self, canvas, point_a, point_b,
                                    point_index):
        if [point_a, point_b] not in Road.connection_list and [point_b,
                                                               point_a] not \
                in Road.connection_list:
...
class Road:
    ...
    def __init__(self, canvas, point_a, point_b, point_index):
        ...
        self.canvas_line = canvas.create_line(self.point_a.x,
                                              self.point_a.y - Point.h,
                                              self.point_b.x,
                                              self.point_b.y - Point.h,
                                              fill='black', width=2)
        canvas.tag_bind(self.canvas_line, '<Button-1>',
                        self.click_road_function)
        Road.canvas = canvas
...
class Player:
    ...
    player_list = []
    id = 0
    colors = {0: 'blue', 1: '#F2D800', 2: 'red', 3: 'purple'}
    sum_dice_throw = None
    move_thieves = False
    is_first_turn = True
    ...
    def alter_chosen_item(self, item):
        if Player.chosen_item == None:
            pass
        elif Player.chosen_item.__class__.__name__ == 'Point' and \
                Player.chosen_item.upgraded == False:
            Point.canvas.itemconfig(Player.chosen_item.point_on_canvas,
                                    outline='black', width=1)
    ...
    def update_player_stats(self):
        ...
        stone_label_text.set(self.stone)
    ...
    def check_if_player_can_build_settelment(self):
        if self.clay >= 1 and self.wheat >= 1 and self.sheep >= 1 and \
                self.tree >= 1:
            self.clay -= 1
            self.wheat -= 1
            self.sheep -= 1
...
class OpeningScreen:
    ...
    def __init__(self, open_screen):
        ...
        self.open_screen_label = Label(open_screen,
                                       text='Number of Players(3 or 4): ')
        ...
        self.open_screen_button.bind('<Button-1>',
                                     self.open_screen_get_number_of_players)
...
def build_button_change_according_to_chosen_item():
    ...
    if type == 'Road':
        build_settlement_button_text.set('Build Road')
    elif type == 'Point':
        if Player.chosen_item.player == None:
            build_settlement_button_text.set('Build City')
        elif Player.chosen_item.upgraded == False:
            build_settlement_button_text.set('Upgrade City')
        else:
            build_settlement_button_text.set('City Upgraded')
    else:
        build_settlement_button_text.set('No Item Chosen')


def throw_dice(event):
    throw_dice_button["state"] = "disabled"
    build_settlement_button["state"] = "normal"
    end_turn_button["state"] = "normal"
    development_card_button["state"] = "normal"
                ...
                if hexagon.dice_throw == Player.sum_dice_throw:
                    for point in hexagon.points:
                        if point.player != None:
                            if hexagon.type == 'wheat' and \
                                    Hexagon.thieves != hexagon:
                                point.player.wheat += 1
                            elif hexagon.type == 'clay' and \
                                    Hexagon.thieves != hexagon:
                                point.player.clay += 1
                            elif hexagon.type == 'stone' and \
                                    Hexagon.thieves != hexagon:
                                point.player.stone += 1
                            elif hexagon.type == 'sheep' and \
                                    Hexagon.thieves != hexagon:
                                point.player.sheep += 1
                            elif hexagon.type == 'tree' and \
                                    Hexagon.thieves != hexagon:
                                point.player.tree += 1
...
def check_if_player_can_build(event):
    if build_settlement_button["state"] == "normal":
        if Player.chosen_item.__class__.__name__ == 'Point':
            if Player.chosen_item.player == None and \
                    Player.chosen_item.can_build_on == True:

                if Player.is_first_turn == True:
                    Player.chosen_item.player = Player.current_player
                    Player.chosen_item.canvas.itemconfig(
                        Player.chosen_item.point_on_canvas,
                        fill=Player.colors[Player.current_player.id])
                    Player.chosen_item.disable_close_points()
                    Player.current_player.points += 1
                    Player.current_player.update_player_stats()
                    Player.is_first_turn = False
                    end_turn_button["state"] = "normal"
                    build_settlement_button["state"] = "disabled"

                elif Player.is_second_turn == True:
                    pass
                elif Player.current_player.check_if_player_can_build_settelment()\
                        == True:
                    print(Player.is_first_turn,Player.is_second_turn)
                    Player.chosen_item.player = Player.current_player
                    Player.chosen_item.canvas.itemconfig(
                        Player.chosen_item.point_on_canvas,
                        fill=Player.colors[Player.current_player.id])
            ...
            elif Player.current_player.chosen_item.upgraded == False and \
                    Player.current_player.check_if_player_can_upgrade():
                Player.chosen_item.upgraded = True
                Player.current_player.points += 1
                Point.canvas.itemconfig(Player.chosen_item.point_on_canvas,
                                        outline=Player.colors[
                                            Player.current_player.id], width=8)
                build_settlement_button_text.set("City Upgraded")
                Player.current_player.update_player_stats()
        ...
        if Player.chosen_item.player == None:
            ...
            if Player.is_first_turn == True and Player.current_player.points \
                    == 1 and (
                    Player.chosen_item.point_a.player ==
                    Player.current_player or
                    Player.chosen_item.point_b.player == Player.current_player):
                Player.chosen_item.player = Player.current_player
                Player.chosen_item.canvas.itemconfig(
                    Player.chosen_item.canvas_line,
                    fill=Player.colors[Player.current_player.id], width="10")
            ...
            elif Player.is_second_turn == True and \
                    Player.current_player.points == 2 and (
                    Player.chosen_item.point_a.player ==
                    Player.current_player or
                    Player.chosen_item.point_b.player == Player.current_player):
                Player.chosen_item.player = Player.current_player
            ...
            # Normal Turn:
            elif Player.is_first_turn == False and Player.is_second_turn == \
                    False:
                if Player.chosen_item.point_a.player == Player.current_player\
                        or Player.chosen_item.point_b.player == \
                        Player.current_player:
                    if Player.current_player.check_if_player_can_build_road()\
                            == True:
                        Player.chosen_item.player = Player.current_player
                        Player.chosen_item.canvas.itemconfig(
                            Player.chosen_item.canvas_line,
                            ...
                            width="10")
                        Player.current_player.roads += 1
                        Player.current_player.roads_list.append(
                            Player.chosen_item)
                        Player.check_who_has_longest_road(Player)
                        Player.current_player.update_player_stats()

                elif Player.chosen_item.point_a.check_if_continued_road() == \
                        True and Player.is_second_turn == False:
                    if Player.current_player.check_if_player_can_build_road()\
                            == True:
                        Player.chosen_item.player = Player.current_player
                        Player.chosen_item.canvas.itemconfig(
                ...
                elif Player.chosen_item.point_b.check_if_continued_road() == \
                        True and Player.is_second_turn == False:
                    if Player.current_player.check_if_player_can_build_road()\
                            == True:
                        Player.chosen_item.player = Player.current_player
...
def end_turn_function(event):
    if end_turn_button["state"] != "disabled":
        ...
        if Player.is_first_turn == False and Player.is_second_turn == False:
            ...
            if Player.current_player.id == len(Player.player_list) - 1:
                ...
                turn_of_lbl2.configure(
                    fg=Player.colors[Player.current_player.id])

            else:
                Player.current_player = Player.player_list[
                    Player.current_player.id + 1]
                turn_of_lbl2.configure(
                    fg=Player.colors[Player.current_player.id])
            # turn_of_txt.set("Turn of:\n" + Player.colors[
            ...
            Player.current_player.check_if_winner()
            throw_dice_button["state"] = "normal"
        elif Player.is_first_turn and Player.is_second_turn == False:
            Player.is_first_turn = False
            Player.is_second_turn = True
        elif Player.is_first_turn == False and Player.is_second_turn == True:
            Player.is_second_turn = False

        end_turn_button["state"] = "disabled"
        build_settlement_button["state"] = "disabled"
        development_card_button["state"] = "disabled"
...
def end_turn_function_without_event():
    ...
    elif Player.is_first_turn == True and Player.is_second_turn == True:  #
        # Another Round for the last player
        Player.is_first_turn = False
...
hexagon_canvas.configure(bg="#62B1F0")
###############################################################
...
print(Hexagon.print_hexagon_board(Hexagon))
Point.raise_tag_for_all_poins(Point, hexagon_canvas)
button_frame = Frame(root)
###############################################################
turn_of_txt1 = StringVar()
turn_of_txt2 = StringVar()

turn_of_txt1.set("Turn of: ")
turn_of_txt2.set("⚫")

turn_of_lbl1 = Label(button_frame, textvariable=turn_of_txt1)
turn_of_lbl2 = Label(button_frame, textvariable=turn_of_txt2, borderwidth=2,
                     relief="solid")

turn_of_lbl2.configure(fg=Player.colors[Player.current_player.id],
                       font=("Arial", 40))

turn_of_lbl1.grid(row=0, column=0)
turn_of_lbl2.grid(row=0, column=1, columnspan=2, pady=10, padx=5)

throw_dice_button = Button(button_frame, text='Throw Dice')
throw_dice_button.bind('<Button-1>', throw_dice)
throw_dice_button["state"] = "disabled"
throw_dice_button.grid(row=1, column=0, pady=10, padx=5)
...
build_settlement_button_text = StringVar()
build_settlement_button_text.set('Build City')
build_settlement_button = Button(button_frame, textvariable=build_settlement_button_text)
build_settlement_button["state"] = "normal"
build_settlement_button.grid(row=3, column=0, pady=10, padx=5)
build_settlement_button.bind('<Button-1>', check_if_player_can_build)


development_card_button = Button(button_frame, text='Development Card')
development_card_button["state"] = "disabled"
development_card_button.grid(row=5, column=0, pady=10, padx=5)

end_turn_button = Button(button_frame, text='End Turn')
end_turn_button.bind('<Button-1>', end_turn_function)
end_turn_button.configure(state="disabled")
end_turn_button.grid(row=6, column=0, pady=10, padx=5)
